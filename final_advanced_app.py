...
def fetch_stock_data(ticker, start_date, end_date):
    try:
        adjusted_ticker = ticker.replace(".", "-")  # Adjust special tickers
...
def analyze_single_stock(stock, current_price, predicted_price):
    ...
    try:
        ...
        logging.error(f"Error analyzing options for {stock.info.get('shortName', 'Unknown Stock')}: {e}")
...
def find_top_recommendations():
    ...
    for ticker in top_50_stocks:
        try:
            ...
            time.sleep(0.1)  # Rate limit safeguard
        except Exception as e:
    ...
    for ticker, current_price, predicted_price, movement in sorted_results:
        try:
            ...
            logging.error(f"Error analyzing options for ticker '{ticker}': {e}")
...
try:
    ...
    logging.error(f"Global error: {traceback.format_exc()}")
    st.error("An unexpected error occurred. See details in the log file.")
