...
def fetch_stock_data(ticker, start_date, end_date):
    try:
        # Adjust special tickers (e.g., BRK.B to BRK-B for Yahoo Finance)
...
def analyze_single_stock(stock, current_price, predicted_price):
    st.subheader(f"Options Recommendations for {stock.info['shortName']}")
    try:
        if not stock.options:
            st.warning(f"No options data available for {stock.info['shortName']}.")
            return

        # Fetch options data
        expiry_dates = stock.options
        expiry = expiry_dates[0]  # Use the nearest expiry date for simplicity
        options_chain = stock.option_chain(expiry)
        calls = options_chain.calls
        puts = options_chain.puts

        # Filter options near the current price
        ...
        if predicted_price > current_price * 1.05:
            st.success(
                f"Recommendation: BUY CALL OPTIONS - Predicted price is ${predicted_price:.2f}, up from current price ${current_price:.2f}. "
                "Call options provide profit opportunities in this upward trend."
            )
        elif predicted_price < current_price * 0.95:
            st.success(
                f"Recommendation: BUY PUT OPTIONS - Predicted price is ${predicted_price:.2f}, down from current price ${current_price:.2f}. "
                "Put options provide profit opportunities in this downward trend."
            )
        else:
...
def find_top_recommendations():
    ...
    for ticker in top_50_stocks:
        try:
            ...
            # Calculate movement and add to results
    ...
    if not valid_results:
        st.warning("No valid recommendations could be generated due to missing or insufficient data.")
        return
